...
def check_fixversion_exists(project, fixversion):
    ...
    if fixversion not in fixversions:
        print 'Warning: Release %s was not found in project %s ' % (CURRENT_RELEASE, PROJECT)
        exit(1)
...
print 'Querying project %s for issues in release %s...' % (PROJECT, CURRENT_RELEASE)

check_fixversion_exists(PROJECT, CURRENT_RELEASE)

initial_items = jira.search_issues('fixVersion = %s and created <= %s and type in (bug,task)' % (CURRENT_RELEASE, PLAN_DATE))
addl_items = jira.search_issues('fixVersion = %s and created > %s and type in (bug,task)' % (CURRENT_RELEASE, PLAN_DATE))
...
non_release_items = jira.search_issues('labels in (re-related) AND resolved >= %s and Status in (Finished, Done) ORDER BY created ASC' % PLAN_DATE)
